...
def news_checker(request):
    ...
    if((url is not None) and (len(url) > 5)):
        ...
        log_model = pickle.load(
            open('E:/Deployed/thecodejournalists/backend/apps/checker/log_model.sav', 'rb'))
        cDict = loadCanonDict()
        ss = SoupStrainer()
        ss.init()
        ...
        fin_prb = log_prb
        fin_totFake = log_totFake
        fin_totReal = log_totReal
        ...
        with open(f'E:/Deployed/thecodejournalists/frontend/assets/images/wordclouds/{imgname}', 'wb') as f:
